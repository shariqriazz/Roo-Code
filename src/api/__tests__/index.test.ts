...
describe("getModelParams", () => {
	...
	it("should use _modeTemperature when provided", () => {
		const options = { _modeTemperature: 0.8, modelTemperature: 0.5 }
		const model = {
			id: "test-model",
			contextWindow: 16000,
			supportsPromptCache: true,
		...
		expect(result).toEqual({
			maxTokens: 1000,
			thinking: undefined,
			temperature: 0.8, // Should use _modeTemperature instead of modelTemperature or defaultTemperature
		})
	})

	it("should convert null _modeTemperature to undefined when provided", () => {
		const options = { _modeTemperature: null, modelTemperature: 0.5 }
		const model = {
			id: "test-model",
			contextWindow: 16000,
			supportsPromptCache: true,
		...
		expect(result).toEqual({
			maxTokens: 1000,
			thinking: undefined,
			temperature: undefined, // Should convert null to undefined
		})
	})
})
