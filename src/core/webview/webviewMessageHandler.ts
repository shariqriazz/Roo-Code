...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "setModeTemperature": {
			if (message.mode && message.temperature !== undefined) {
				await provider.providerSettingsManager.setModeTemperature(message.mode as Mode, message.temperature as number | null)

				// If this is the current mode, update the API handler with the new temperature
				const currentMode = getGlobalState("mode")
				if (currentMode === message.mode && provider.getCurrentCline()) {
					const apiConfiguration = provider.contextProxy.getProviderSettings()

					// Rebuild the API handler with the mode-specific temperature
					const apiConfig = {
						...apiConfiguration,
					}

					// Add the mode temperature as a custom property
					// @ts-ignore - We're adding a custom property that's handled in getModelParams
					apiConfig._modeTemperature = message.temperature

					provider.getCurrentCline()!.api = buildApiHandler(apiConfig)
				}

				// Update the modeTemperatures in global state
				const modeTemperatures = getGlobalState("modeTemperatures") || {}
				modeTemperatures[message.mode as Mode] = message.temperature as number | null
				await updateGlobalState("modeTemperatures", modeTemperatures)
