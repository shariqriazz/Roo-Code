...
import { providerSettingsSchema, ApiConfigMeta, ModeTemperatureSettings } from "../../schemas"
...
export class ProviderSettingsManager {
	...
	/**
	 * Set the temperature for a specific mode.
	 */
	public async setModeTemperature(mode: Mode, temperature: number | null | undefined) {
		try {
			return await this.lock(async () => {
				// Get current mode temperatures from global state
				const modeTemperatures = await this.context.globalState.get<ModeTemperatureSettings>("modeTemperatures") || {}

				// Update the temperature for the specified mode
				modeTemperatures[mode] = temperature

				// Save the updated mode temperatures
				await this.context.globalState.update("modeTemperatures", modeTemperatures)
			})
		} catch (error) {
			throw new Error(`Failed to set mode temperature: ${error}`)
		}
	}

	/**
	 * Get the temperature for a specific mode.
	 */
	public async getModeTemperature(mode: Mode): Promise<number | null | undefined> {
		try {
			// Get current mode temperatures from global state
			const modeTemperatures = await this.context.globalState.get<ModeTemperatureSettings>("modeTemperatures") || {}

			// Return the temperature for the specified mode
			return modeTemperatures[mode]
		} catch (error) {
			throw new Error(`Failed to get mode temperature: ${error}`)
		}
	}
